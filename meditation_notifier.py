...
class MeditationNotifier:
    def __init__(self):
        ...
        self.config_dir = Path.home() / '.meditation-notifier'
    ...
    def get_current_principle(self):
        start_date = datetime.fromisoformat(self.config['start_date'])
        current_week = (datetime.now() - start_date).days // 7
        index = current_week % len(self.principles)
        return f"Week {current_week + 1}:\n{self.principles[index]}"

    def send_notification(self):
        principle = self.get_current_principle()
        principle = principle.replace('"', '\\"')

        # Display dialog with a beep
        applescript = f'''
        tell application "System Events"
            beep
            display dialog "{principle}" with title "Weekly Meditation on Principles" with icon 1 buttons {{"Close"}} default button "Close"
        end tell
        '''
        subprocess.run(['osascript', '-e', applescript])

if __name__ == "__main__":
    notifier = MeditationNotifier()
